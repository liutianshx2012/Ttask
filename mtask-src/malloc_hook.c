...
void *
mtask_aligned_alloc(size_t alignment, size_t size)
{
...
int
mtask_posix_memalign(void **memptr, size_t alignment, size_t size)
{
...
mallctl_bool(const char* name, bool* newval)
{
    mtask_error(NULL, "No jemalloc : mallctl_bool %s.", name);
    return 0;
}
...
mallctl_cmd(const char* name)
{
    mtask_error(NULL, "No jemalloc : mallctl_cmd %s.", name);
    return 0;
}
...
dump_c_mem(void)
{
	...
	for(i=0; i<SLOT_SIZE; i++) {
		...
		if(data->handle != 0 && data->allocated != 0) {
            ...
            mtask_error(NULL, ":%08x -> %zdkb %db", data->handle, data->allocated >> 10 , (int)(data->allocated % 1024));
		}
	}
