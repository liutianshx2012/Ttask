...
void
socket_server_updatetime(socket_server_t *ss, uint64_t time)
{
    ss->time = time;
}
...
_force_close(socket_server_t *ss, struct socket *s, struct socket_lock *l ,socket_message_t *result)
{
	...
	s->type = SOCKET_TYPE_INVALID;
    if (s->dw_buffer) {
        _free_buffer(ss, s->dw_buffer, (int)s->dw_size);
        s->dw_buffer = NULL;
    }
    socket_unlock(l);
}
...
static inline void
stat_read(socket_server_t *ss, struct socket *s, int n)
{
    s->stat.read += n;
...
static inline void
stat_write(socket_server_t *ss, struct socket *s, int n)
{
    s->stat.write += n;
...
static void
drop_udp(socket_server_t *ss, struct socket *s, struct wb_list *list, struct write_buffer *tmp)
{
    s->wb_size -= tmp->sz;
    list->head = tmp->next;
    if (list->head == NULL)
...
static inline int
_nomore_sending_data(struct socket *s)
{
    return _send_buffer_empty(s) && s->dw_buffer == NULL && (s->sending & 0xffff) == 0;
}
...
_close_socket(socket_server_t *ss, struct request_close *request, socket_message_t *result)
{
    ...
    struct socket_lock l;
    socket_lock_init(s, &l);
	if (!_nomore_sending_data(s)) {
		int type = _send_buffer(ss,s,&l,result);
		// type : -1 or SOCKET_WARNING or SOCKET_CLOSE, SOCKET_WARNING means _nomore_sending_data
		if (type != -1 && type != SOCKET_WARNING)
			return type;
	}
	if (request->shutdown || _nomore_sending_data(s)) {
		_force_close(ss,s,&l,result);
		result->id = id;
		result->opaque = request->opaque;
		return SOCKET_CLOSE;
...
static inline void
dec_sending_ref(socket_server_t *ss, int id)
{
    struct socket * s = &ss->slot[HASH_ID(id)];
...
_ctrl_cmd(socket_server_t *ss, socket_message_t *result) 
{
	...
	switch (type) {
        ...
        case 'P': {
            ...
            int ret = _send_socket(ss, request, result, priority, NULL);
...
{
    return s->id == id && _nomore_sending_data(s) && s->type == SOCKET_TYPE_CONNECTED
                &&  s->udpconnecting == 0;
}
...
mtask_socket_info_create(struct mtask_socket_info *last)
{
    struct mtask_socket_info *si = mtask_malloc(sizeof(*si));
...
void
mtask_socket_info_release(struct mtask_socket_info *si)
{
    while (si) {
        struct mtask_socket_info *temp = si;
        si = si->next;
        mtask_free(temp);
    }
}
...
static int
query_info(struct socket *s, struct mtask_socket_info *si)
{
    union sockaddr_all u;
    socklen_t slen = sizeof(u);
    switch (s->type) {
...
struct mtask_socket_info *
socket_server_info(socket_server_t *ss)
{
    int i;
    struct mtask_socket_info * si = NULL;
    for (i=0;i<MAX_SOCKET;i++) {
        struct socket * s = &ss->slot[i];
        int id = s->id;
        struct mtask_socket_info temp;
        if (query_info(s, &temp) && s->id == id) {
            // socket_server_info may call in different thread, so check socket id again
            si = mtask_socket_info_create(si);
            temp.next = si->next;
            *si = temp;
        }
    }
