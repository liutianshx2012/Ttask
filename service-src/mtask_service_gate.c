...
_forward(struct gate *g, struct connection * c, int size)
{
    ...
    if (g->broker) {
        ...
        mtask_send(ctx, 0, g->broker, g->client_tag | PTYPE_TAG_DONTCOPY, fd, temp, size);
        return;
    }
    if (c->agent) {
        void * temp = mtask_malloc(size);
        databuffer_read(&c->buffer,&g->mp,temp, size);
		mtask_send(ctx, c->client, c->agent, g->client_tag | PTYPE_TAG_DONTCOPY, fd , temp, size);
    } else if (g->watchdog) {
        char * tmp = mtask_malloc(size + 32);
        int n = snprintf(tmp,32,"%d data ",c->id);
        databuffer_read(&c->buffer,&g->mp,tmp+n,size);
		mtask_send(ctx, 0, g->watchdog, PTYPE_TEXT | PTYPE_TAG_DONTCOPY, fd, tmp, size + n);
    }
}
