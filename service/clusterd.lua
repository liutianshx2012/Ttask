...
local function open_channel(t, key)
	...
	if address == nil and not config.nowaiting then
		...
		ct.namequery = co
		mtask.error("Waiting for cluster node [".. key.."]")
		mtask.wait(co)
	...
	if address then
		...
		c = node_sender[key]
		if c == nil then
			c = mtask.newservice("clustersender", key, nodename)
		...
		succ = pcall(mtask.call, c, "lua", "changenode", host, port)
...
local function loadconfig(tmp)
	...
	for name,address in pairs(tmp) do
		if name:sub(1,2) == "__" then
			name = name:sub(3)
			config[name] = address
			mtask.error(string.format("Config %s = %s", name, address))
		else
			...
			if ct and ct.namequery and not config.nowaiting then
				mtask.error(string.format("Cluster node [%s] resloved : %s", name, address))
				mtask.wakeup(ct.namequery)
			end
		end
	end
...
function command.sender(source, node)
	mtask.ret(mtask.pack(node_channel[node]))
end
...
function command.proxy(source, node, name)
	...
	if p == nil then
		p = mtask.newservice("clusterproxy", node, name)
		-- double check
...
local function clearnamecache()
	for fd, service in pairs(cluster_agent) do
		if type(service) == "number" then
			mtask.send(service, "lua", "namechange")
		end
	end
end
...
function command.queryname(source, name)
	mtask.ret(mtask.pack(register_name[name]))
end
...
function command.socket(source, subcmd, fd, msg)
	if subcmd == "open" then
		...
		local agent = mtask.newservice("clusteragent", mtask.self(), source, fd)
		local closed = cluster_agent[fd]
		cluster_agent[fd] = agent
		if closed then
			mtask.send(agent, "lua", "exit")
		...
		if subcmd == "close" or subcmd == "error" then
			...
			mtask.error(string.format("socket %s %d %s", subcmd, fd, msg or ""))
		end
	end
end
