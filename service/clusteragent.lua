...
local ignoreret = mtask.ignoreret
...
local register_name_mt = { __index =
	function(self, name)
		...
		if waitco then
			...
			local addr = mtask.call(clusterd, "lua", "queryname", name:sub(2))	-- name must be '@xxxx'
			if addr then
				self[name] = addr
			end
...
local function dispatch_request(_,_,addr, session, msg, sz, padding, is_push)
	ignoreret()	-- session is fd, don't call mtask.ret
	if session == nil then
		-- trace
		tracetag = addr
		return
	...
	if addr == 0 then
		...
		if addr then
			if is_push then
				...
				if tracetag then
					ok , msg, sz = pcall(mtask.tracecall, tracetag, addr, "lua", msg, sz)
					tracetag = nil
				else
...
mtask.start(function()
	...
	mtask.dispatch("lua", function(_,source, cmd, ...)
		...
		elseif cmd == "namechange" then
			...
			mtask.error(string.format("Invalid command %s from %s", cmd, mtask.address(source)))
		end
	end)
end)
