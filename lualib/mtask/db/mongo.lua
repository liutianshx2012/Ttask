...
local socketchannel	= require "mtask.socketchannel"
local mtask = require "mtask"
local driver = require "mtask.mongo.driver"
		...
		if rs_data.ok == 1 then
			...
			if rs_data.ismaster	then
				...
				if rs_data.primary then
					...
					mtask.error("WARNING: NO PRIMARY RETURN " .. rs_data.me)
					...
					mtask.error("INFO: TRY TO CONNECT " .. mongoc.__pickserver[1])
...
function mongo_client:auth_scram_sha1(username,password)
	...
	if not string.sub(rnonce, 1, 12) == nonce then
		mtask.error("Server returned an invalid nonce.")
		return false
	end
	...
	if parsed_t['v'] ~= server_sig then
		mtask.error("Server returned an invalid signature.")
		return false
	end
	if not r.done then
		...
		if not r.done then
			mtask.error("SASL conversation failed to complete.")
			return false
		end
	end
	return true
