...
local function connect_once(self)
			...
			if id == self.__sock[1] then
				if size == 0 then
					...
					if not self.__overload then
						...
						mtask.error(string.format("WARNING: %d K bytes need to send out (fd = %d %s:%s)", size, id, self.__host, self.__port))
					end
				end
			end
		end

		mtask.fork(overload_trigger, fd, 0)
	...
	self.__dispatch_thread = mtask.fork(function()
